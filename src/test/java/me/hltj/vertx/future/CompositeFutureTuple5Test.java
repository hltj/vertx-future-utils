...
class CompositeFutureTuple5Test {
    ...
    void basic() {
        ...
        val tuple = tuple(future0, future1, future2, future3, future4);
        ...
        assertEquals(
                "CompositeFutureTuple5(FutureTuple5(Future{result=hello}, Future{unresolved}, " +
                        "Future{result=null}, Future{result=null}, Future{result=null}))",
                composite.toString()
        );
    }
    ...
    void use() {
        ...
        join(promise0.future(), future1, future2, future3, future4).use((composite, fut0, fut1, fut2, fut3, fut4) ->
    ...
    void with() {
        ...
        Future<String> resultFuture = join(promise0.future(), future1, future2, future3, future4)
                .with((composite, fut0, fut1, fut2, fut3, fut4) -> composite.map(_t -> {
                    SharedTestUtils.assertSucceedWith(1.0, fut0);
                    ...
                    return String.format("(%s, %s, %s, %s, %s)", fut0, fut1, fut2, fut3, fut4);
                }));
    ...
    void through_mapAnyway() {
        ...
        val composite = join(promise0.future(), promise1.future(), future2, future3, future4);

        Future<Double> sumFutureA = composite.through((fut0, fut1, fut2, fut3, fut4) ->
        ...
        Future<Double> sumFutureB = composite.mapAnyway((fut0, fut1, fut2, fut3, fut4) ->
                fallbackWith(fut0, 0.0).result() +
                        fallbackWith(fut1, 0).result() +
                        fallbackWith(fut2, 0L).result() +
                        fallbackWith(fut3, 0.0).result() +
    ...
    void through_mapAnyway_NPE() {
        ...
        Future<Double> sumFutureA = composite.through((fut0, fut1, fut2, fut3, fut4) ->
                fut0.result() + fut1.result() + fut2.result() + fut3.result() + fut4.result()
        );
    ...
    void joinThrough_flatMapAnyway() {
        ...
        Future<Double> sumFutureA = composite.joinThrough((fut0, fut1, fut2, fut3, fut4) -> wrap(() ->
                fallbackWith(fut0, 0.0).result() +
                        fallbackWith(fut1, 0).result() +
                        fallbackWith(fut2, 0L).result() +
                        fallbackWith(fut3, 0.0).result() +
        ...
        Future<Double> sumFutureB = composite.flatMapAnyway((fut0, fut1, fut2, fut3, fut4) -> wrap(() ->
                fallbackWith(fut0, 0.0).result() +
                        fallbackWith(fut1, 0).result() +
                        fallbackWith(fut2, 0L).result() +
                        fallbackWith(fut3, 0.0).result() +
    ...
    void joinThrough_flatMapAnyway_NPE() {
        ...
        Future<Double> sumFutureA = composite.joinThrough((fut0, fut1, fut2, fut3, fut4) ->
        ...
        Future<Double> sumFutureB = composite.flatMapAnyway((fut0, fut1, fut2, fut3, fut4) ->
                wrap(null, (Double init) -> init +
                        fallbackWith(fut0, 0.0).result() +
                        fallbackWith(fut1, 0).result() +
                        fallbackWith(fut2, 0L).result() +
        ...
        Future<Double> sumFutureC = composite.joinThrough((fut0, fut1, fut2, fut3, fut4) ->
        ...
        @SuppressWarnings("ConstantConditions")
        Future<Double> sumFutureD = composite.flatMapAnyway((fut0, fut1, fut2, fut3, fut4) ->
                ((Future<Integer>) null).map(
                        fallbackWith(fut0, 0.0).result() +
                                fallbackWith(fut1, 0).result() +
                                fallbackWith(fut2, 0L).result() +
    ...
    void applift_mapTyped() {
        ...
        Future<Double> future3 = Future.succeededFuture(0.0);
        Future<Character> future4 = Future.succeededFuture('\0');
        val composite = tuple(promise0.future(), promise1.future(), future2, future3, future4)
                .fallback(0.0, 0, 0L, 0.0, '\0').all();

        Future<Double> sumFutureA = composite.applift((d0, i1, l2, d3, c4) -> d0 + 1.0 * i1 + l2 + d3 + c4);
        Future<Double> sumFutureB = composite.mapTyped((d0, i1, l2, d3, c4) -> d0 + 1.0 * i1 + l2 + d3 + c4);
    ...
    void joinApplift_flatMapTyped() {
        ...
        Future<Double> sumFutureA = composite
                .joinApplift((d0, i1, l2, d3, c4) -> wrap(() -> d0 + 1.0 * i1 + l2 + d3 + c4));
        Future<Double> sumFutureB = composite
                .flatMapTyped((d0, i1, l2, d3, c4) -> wrap(() -> d0 + 1.0 * i1 + l2 + d3 + c4));
    ...
    void joinApplift_flatMapTyped_Failure() {
        ...
        Future<Double> future3 = Future.succeededFuture(0.0);
        Future<Character> future4 = Future.succeededFuture('\0');
        val compositeA = join(promise0.future(), promise1.future(), future2, future3, future4);
        val compositeC = tuple(promise0.future(), promise1.future(), future2, future3, future4)
                .otherwise(0.0, 0, 0L, 0.0, '\0').all();
        ...
        Future<Double> sumFutureC = compositeC
                .joinApplift((d0, i1, l2, d3, c4) -> ((Future<Integer>) null).map(d0 + i1 + l2 + d3 + c4));
        @SuppressWarnings("ConstantConditions")
        Future<Double> sumFutureD = compositeC
                .flatMapTyped((d0, i1, l2, d3, c4) -> ((Future<Integer>) null).map(d0 + i1 + l2 + d3 + c4));
